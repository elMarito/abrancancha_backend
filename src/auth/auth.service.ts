...
import { ServiceResponseOk, today, urlSafeHash, urlUnsafeHash } from 'src/utilities';
...
export class AuthService {
  ...
  async resetPassword(email: string): Promise<any> {
    // try {
      if (!email) throw new BadRequestException("El email proporcionado no es valido");
      const user = await this.userService.getUserByEmail(email);
      // console.log("++++",user);
      if (!user)
      ...
      return ServiceResponseOk('La contraseña a sido reseteada y el token es valido hasta medianoche.',{ resetToken: safehash })
  ...
  async changePassword(resetToken: string, newPassword: string) {
    ...
    if (!isPasswordValid)
      throw new UnauthorizedException('El link de reseteo a caducado, genere uno nuevo');
